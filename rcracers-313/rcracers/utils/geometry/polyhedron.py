...
class LPRepresentation:
    ...
    @property
    def matrix(self) -> cdd.Matrix:
        return self.matrix

    def canonicalize(self):
        cdd.matrix_canonicalize(self.matrix)
        # self.matrix.canonicalize()
        return self

    @property
    def size(self):
        return len(self.matrix.array[0]) - 1

    @property
    def nb_constraints(self):
        return len(self.matrix.array)

    @property
    def representation(self):
        return np.array(self.matrix.array)
...
class H_Representation(LPRepresentation):
    ...
    def matrix(self):
        if self._matrix is None:
            self._matrix = cdd.matrix_from_array(
                H_Representation.to_cdd(self._inequalities)
            )
    ...
    @property
    def representation(self):
        return H_Representation.from_cdd(self.matrix.array)
...
class V_Representation(LPRepresentation):
    ...
    def matrix(self):
        if self._matrix is None:
            ...
            self._matrix = cdd.matrix_from_array(self.to_cdd(stack))
            self._matrix.rep_type = cdd.RepType.GENERATOR
            if np.size(self._linspan) > 0:
                self._matrix.extend(
                    self.to_cdd(
                        np.hstack(
                            [np.zeros((self._linspan.shape[0], 1)), self._linspan]
                        )
                    ),
                    linear=True,
                )
...
class Polyhedron:
    ...
    def _polyhedron(self):
        if self._backend is None:
            self._backend = cdd.polyhedron_from_matrix(self.description.matrix)
        return self._backend

    def constraints(self):
        if self._h_rep is None:
            self._h_rep = H_Representation(cdd.copy_inequalities(self._polyhedron()))
        return self._h_rep

    def generators(self):
        if self._v_rep is None:
            poly: cdd.Polyhedron = self._polyhedron()
            res = cdd.copy_generators(poly)
            if len(res.array) == 0:
    ...
    def get_vertex_adjacency(self):
        if isinstance(self.description, H_Representation):
            return cdd.copy_adjacency(self._polyhedron())
        else:
            return cdd.copy_input_adjacency(self._polyhedron())
    ...
    def get_vertex_incidence(self):
        if isinstance(self.description, H_Representation):
            return cdd.copy_incidence(self._polyhedron())
        else:
            return cdd.copy_input_incidence(self._polyhedron())
    ...
    def nb_active(self, other: np.ndarray, axis: int = -1, tol: float = 1e-10):
        ...
        i_check = (
            np.abs(np.einsum("ij,...j->i...", normals, other) + offset[..., np.newaxis])
            <= tol
        )
    ...
    def _contains_poly(self, other: "Polyhedron", tol: float):
        if other.has_vrep:
            ...
            return np.all(
                [self.contains(vertex, tol=tol) for vertex in other.vertices()]
            )
        if self.has_vrep:
            return not np.any(
                [other.contains(vertex, tol=tol) for vertex in other.vertices()]
            )
        ...
        if n > 0:
            A_eq = np.hstack([A_eq, np.kron(other.equalities()[:, :-1].T, np.eye(r))])
            A_ub = np.hstack([A_ub, -np.kron(other.equalities()[:, -1::].T, np.eye(r))])
        ...
        b_ub = np.concatenate(
            [
                b_ub + reltol(b_ub, tol),
                b_eq + reltol(b_eq, tol),
                -b_eq + reltol(b_eq, tol),
            ]
        )
        A_eq = None
    ...
    def contains(
        ...
        other = np.moveaxis(other, source=axis, destination=-1)
        i_check = np.all(
            np.einsum("ij,...j->i...", normals, other) + offset[..., np.newaxis]
            <= reltol(offset, tol),
            axis=0,
        )
        e_check = np.all(
    ...
    def gauge(self, vector: np.ndarray):
        ...
        if self._h_rep is None:
            ...
            res = linprog(
                cost, A_eq=normals, b_eq=offset, bounds=bounds, method="revised simplex"
            )
            return res.fun
    ...
    def image(self, transformation: np.ndarray, inverse: bool = False):
        ...
        if inverse:
            ...
            return Polyhedron.from_constraints(
                np.hstack(
                    [
                        self.inequalities()[:, :-1] @ transformation[:-1, :-1],
                        self.inequalities()[:, :-1] @ transformation[:-1, -1:]
                        + self.inequalities()[:, -1:],
                    ]
                ),
                np.hstack(
                    [
                        self.equalities()[:, :-1] @ transformation[:-1, :-1],
                        self.equalities()[:, :-1] @ transformation[:-1, -1:]
                        + self.equalities()[:, -1:],
                    ]
                ),
            ...
            return Polyhedron.from_generators(
                self.vertices() @ transformation[:-1, :-1].T
